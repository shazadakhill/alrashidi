...
class EmployeeAssignedApplications extends Component {
    componentDidMount() {
        window.scrollTo(0, 0)
        this.mounted = true;

        const token = 'Bearer '.concat(this.props.loginProps.token);
        axios.get(`${ApiURL.domain}${ApiURL.applications}?assigned=1`,{ "headers": { Authorization: token } }).then(
            response => {
                if (this.mounted) {
                    this.setState({
    ...
    const { t } = this.props;
        ...
        return (
            <div className="servicesSubServices">
                <Heading type="h1" > {t("employeeAssignedApplications.heading1")}</Heading>
                {application}
            </div>
        )
    }
...
export default withTranslation()(connect(mapStateToProps)(EmployeeAssignedApplications));