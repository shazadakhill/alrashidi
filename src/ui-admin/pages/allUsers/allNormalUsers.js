...
class AllNormalUsers extends Component {
    componentDidMount() {
        window.scrollTo(0, 0)
        this.mounted = true;
        ...
        axios.get(`${ApiURL.domain}${ApiURL.accountsNormalUser}`, { "headers": { Authorization: token } }).then(
            response => {
                if (this.mounted) {
                    this.setState({
                        Users: response.data,
...
export default connect(mapStateToProps)(AllNormalUsers);